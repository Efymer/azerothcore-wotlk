...
std::vector<WorldPackets::Auth::RaceClassAvailability> const WorldPackets::Auth::AuthResponse::EmptyAvailableClasses;
...
-----END RSA PRIVATE KEY-----
    ...
    std::unique_ptr<Acore::Crypto::RsaSignature> ConnectToRSA;
    std::unique_ptr<Acore::Crypto::Ed25519> EnterEncryptedModeSigner;
}

bool WorldPackets::Auth::ConnectTo::InitializeEncryption()
{
    std::unique_ptr<Acore::Crypto::RsaSignature> rsa = std::make_unique<Acore::Crypto::RsaSignature>();
...
WorldPacket const* WorldPackets::Auth::ConnectTo::Write()
{
    ...
    Acore::Crypto::RsaSignature rsa(*ConnectToRSA);
    Acore::Crypto::RsaSignature::SHA256 digestGenerator;
...
{
    std::unique_ptr<Acore::Crypto::Ed25519> ed25519 = std::make_unique<Acore::Crypto::Ed25519>();
...
namespace
{
    // 3.4.3.54261: 16-byte RSA-name seed HMACed with SHA-256 over the EnableEncryption message.
    std::array<uint8, 16> constexpr EnableEncryptionSeedRSA = { 0x90, 0x9C, 0xD0, 0x50, 0x5A, 0x2C, 0x14, 0xDD, 0x5C, 0x2C, 0xC0, 0x64, 0x14, 0xF3, 0xFE, 0xC9 };
...
{
    std::array<uint8, 32> toSign = Acore::Crypto::HMAC_SHA256::GetDigestOf(EncryptionKey,
        std::array<uint8, 1>{uint8(Enabled ? 1 : 0)},
        EnableEncryptionSeedRSA);

    Acore::Crypto::Ed25519 ed25519(*EnterEncryptedModeSigner);
    std::vector<uint8> signature;
