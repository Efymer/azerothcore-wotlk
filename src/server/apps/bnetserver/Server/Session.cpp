...
Battlenet::Session::Session(Acore::Net::IoContextTcpSocket&& socket) : BaseSocket(std::move(socket), SslContext::instance()),
...
void Battlenet::Session::Start()
{
    ...
    { {
        std::make_shared<Acore::Net::IpBanCheckConnectionInitializer<Session>>(this),
        std::make_shared<Acore::Net::SslHandshakeConnectionInitializer<Session>>(this),
        std::make_shared<Acore::Net::ReadConnectionInitializer<Session>>(this),
    } };
...
uint32 Battlenet::Session::HandleLogon(authentication::v1::LogonRequest const* logonRequest, std::function<void(ServiceBase*, uint32, ::google::protobuf::Message const*)>& continuation)
{
    ...
    if (!IsValidLocale(logonRequest->locale()))
    {
    ...
    externalChallenge.set_payload(Acore::StringFormat("http{}://{}:{}/bnetserver/login/", !SslContext::UsesDevWildcardCertificate() ? "s" : "",
...
uint32 Battlenet::Session::HandleGenerateWebCredentials(authentication::v1::GenerateWebCredentialsRequest const* request, std::function<void(ServiceBase*, uint32, google::protobuf::Message const*)>& continuation)
{
    ...
    QueueQuery(LoginDatabase.AsyncQuery(stmt).WithPreparedCallback([this, asyncContinuation = std::move(continuation)](PreparedQueryResult result)
    {
        ...
        response.set_web_credentials((*result)[0].Get<std::string>());
...
{
    if (Variant const* identity = Acore::Containers::MapGetValuePtr(params, "Param_Identity"))
    {
    ...
    if (Variant const* clientInfo = Acore::Containers::MapGetValuePtr(params, "Param_ClientInfo"))
    {
    ...
    stmt->SetData(1, uint8(GetLocaleByName(_locale)));
    stmt->SetData(2, _os);
...
{
    if (Variant const* subRegion = Acore::Containers::MapGetValuePtr(params, "Command_LastCharPlayedRequest_v1"))
    {
        ...
        {
            std::vector<uint8> compressed = Battlenet::RealmListRpc::GetRealmEntryJSON(lastPlayerChar->second.RealmId, _build, _gameAccountInfo->SecurityLevel);
...
uint32 Battlenet::Session::GetRealmList(std::unordered_map<std::string, Variant const*> const& params, game_utilities::v1::ClientResponse* response)
{
    ...
    if (Variant const* subRegion = Acore::Containers::MapGetValuePtr(params, "Command_RealmListRequest_v1"))
        subRegionId = subRegion->string_value();

    std::vector<uint8> compressed = Battlenet::RealmListRpc::GetRealmList(_build, _gameAccountInfo->SecurityLevel, subRegionId);
...
{
    if (Variant const* realmAddress = Acore::Containers::MapGetValuePtr(params, "Param_RealmAddress"))
        return Battlenet::RealmListRpc::JoinRealm(realmAddress->uint_value(), _build, _clientInfo.Platform, _clientInfo.Arch, _clientInfo.Type,
            GetRemoteIpAddress(), _clientSecret, GetLocaleByName(_locale), _os, _timezoneOffset, _gameAccountInfo->Name,
            _gameAccountInfo->SecurityLevel, response);
...
uint32 Battlenet::Session::HandleGetAllValuesForAttribute(game_utilities::v1::GetAllValuesForAttributeRequest const* request, game_utilities::v1::GetAllValuesForAttributeResponse* response)
{
    ...
    {
        Battlenet::RealmListRpc::WriteSubRegions(response);
        return ERROR_OK;
    }
...
static inline Optional<Acore::Net::SocketReadCallbackResult> PartialProcessPacket(Battlenet::Session* session, MessageBuffer& inputBuffer)
{
...
Acore::Net::SocketReadCallbackResult Battlenet::Session::ReadHandler()
{
    MessageBuffer& packet = GetReadBuffer();
    while (packet.GetActiveSize() > 0)
    {
        if (Optional<Acore::Net::SocketReadCallbackResult> partialResult = PartialProcessPacket<&Session::ReadHeaderLengthHandler, &Session::_headerLengthBuffer>(this, packet))
            return *partialResult;

        if (Optional<Acore::Net::SocketReadCallbackResult> partialResult = PartialProcessPacket<&Session::ReadHeaderHandler, &Session::_headerBuffer>(this, packet))
            return *partialResult;

        if (Optional<Acore::Net::SocketReadCallbackResult> partialResult = PartialProcessPacket<&Session::ReadDataHandler, &Session::_packetBuffer>(this, packet))
            return *partialResult;
