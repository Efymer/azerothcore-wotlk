...
char* DB2DatabaseLoader::Load(bool custom, uint32& records, char**& indexTable, std::vector<char*>& stringPool, uint32& minId)
{
    ...
    if (PreparedQueryResult maxIdResult = HotfixDatabase.Query(HotfixDatabase.GetPreparedStatement(HotfixDatabaseStatements(_loadInfo->Statement + HOTFIX_MAX_ID_STMT_OFFSET))))
        if (uint32((*maxIdResult)[0].Get<uint64>()) > records)
            indexTableSize = uint32((*maxIdResult)[0].Get<uint64>());
    ...
    do
    {
        ...
        uint32 indexValue = fields[indexField].Get<uint32>();
        bool isNew = false;
                ...
                switch (_loadInfo->Fields[f].Type)
                {
                    ...
                    case FT_BYTE:
                        *((int8*)(&dataValue[offset])) = fields[f].Get<int8>();
                        ...
                        break;
                    case FT_SHORT:
                        *((int16*)(&dataValue[offset])) = fields[f].Get<int16>();
                        offset += 2;
                        break;
                    case FT_LONG:
                        *((int64*)(&dataValue[offset])) = fields[f].Get<int64>();
                        offset += 8;
                        break;
                    case FT_STRING:
                    {
                        ...
                        if (char* str = AddString(&slot->Str[LOCALE_enUS], fields[f].Get<std::string>()))
                    ...
                    case FT_STRING_NOT_LOCALIZED:
                    {
                        ...
                        // Value in database in main table field must be for enUS locale
                        if (char* str = AddString(slot, fields[f].Get<std::string>()))
                            stringPool.push_back(str);
                        else
                            *slot = nullStr;
                    ...
                    default:
                        ABORT("Unknown format character '{}' found in {} meta for field {}",
                            char(_loadInfo->Fields[f].Type), _storageName, _loadInfo->Fields[f].Name);
                        break;
                }
                ++f;
            }
...
void DB2DatabaseLoader::LoadStrings(bool custom, LocaleConstant locale, uint32 records, char** indexTable, std::vector<char*>& stringPool)
{
    ...
    stmt->SetData(0, !custom);
    stmt->SetData(1, localeNames[locale]);
    PreparedQueryResult result = HotfixDatabase.Query(stmt);
    if (!result)
        return;
    ...
    do
    {
        ...
        if (char* dataValue = indexTable[indexValue])
        {
                ...
                for (uint32 z = 0; z < _loadInfo->Meta->Fields[x].ArraySize; ++z)
                {
                    switch (_loadInfo->Fields[fieldIndex].Type)
                    {
                        ...
                        case FT_STRING:
                        {
                            ...
                            if (char* str = AddString(&db2str->Str[locale], fields[1 + stringFieldNumInRecord].Get<std::string>()))
                                stringPool.push_back(str);
                        ...
                        default:
                            ABORT("Unknown format character '{}' found in {} meta for field {}",
                                char(_loadInfo->Fields[fieldIndex].Type), _storageName, _loadInfo->Fields[fieldIndex].Name);
                            break;
                    }
                    ++fieldIndex;
                }
            ...
            LOG_ERROR("sql.sql", "Hotfix locale table for storage {} references row that does not exist {} locale {}!", _storageName, indexValue, localeNames[locale]);
