    ...
    while (!f.isEof())
    {
        ...
        else if (!strcmp(fourcc, "MODI"))
        {
            ...
            for (uint32 i = 0; i < fileDataIdCount; ++i)
            {
                ...
                std::string path = Acore::StringFormat("FILE{:08X}.xxx", DoodadData.FileDataIds[i]);
                if (ExtractSingleModel(path))
...
int WMOGroup::ConvertToVMAPGroupWmo(FILE* output, bool preciseVectorData)
{
    ...
    if (preciseVectorData)
    {
        ...
        if (nIdexes >0)
        {
            // AzerothCore's raw model reader expects 16-bit indices in the INDX block.
            for (uint32 i = 0; i < nIdexes; ++i)
            {
                uint16 idx = static_cast<uint16>(MOVX[i]);
                if (fwrite(&idx, sizeof(uint16), 1, output) != 1)
                {
    ...
    else
    {
        ...
        // write triangle indices (16-bit, as expected by AzerothCore's raw model reader)
        int INDX[] = {0x58444E49, nColTriangles*6+4, nColTriangles*3};
        fwrite(INDX,4,3,output);
        for (int i = 0; i < nColTriangles*3; ++i)
        {
            uint16 idx = static_cast<uint16>(MovxEx[i]);
            fwrite(&idx, sizeof(uint16), 1, output);
        }
...
void MapObject::Extract(ADT::MODF const& mapObjDef, char const* WmoInstName, uint32 mapID, uint32 tileX, uint32 tileY, FILE* pDirfile)
{
    ...
    std::string tempname = Acore::StringFormat("{}/{}", szWorkDirWmo, WmoInstName);
    ...
    bool isGlobalWmo = (tileX == 65 && tileY == 65);
    ...
    uint16 nameSet = mapObjDef.NameSet;
    if (isGlobalWmo)
