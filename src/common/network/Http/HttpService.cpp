...
RequestHandlerResult DispatcherService::HandleRequest(std::shared_ptr<AbstractSocket> session, RequestContext& context)
{
    ...
    std::time_t responseDate = std::chrono::system_clock::to_time_t(std::chrono::system_clock::now());
    context.response.set(boost::beast::http::field::date, Acore::StringFormat("{:%a, %d %b %Y %T GMT}", fmt::gmtime(responseDate)));
    ...
    HttpMethodHandlerMap& handlerMap = [&]() -> HttpMethodHandlerMap&
    {
        switch (method)
        {
            ...
            default:
            {
                ...
                ABORT("Tried to register a handler for unsupported HTTP method {}", methodString);
            }
        }
    }();
...
{
    _inactiveSessionsKillTimer = std::make_unique<boost::asio::steady_timer>(ioContext);
