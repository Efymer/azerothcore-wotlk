...
#ifndef ACORE_NET_NETWORK_THREAD_H
#define ACORE_NET_NETWORK_THREAD_H
...
public:
    ...
    Acore::Net::IoContextTcpSocket* GetSocketForAccept() { return &_acceptSocket; }
...
protected:
    ...
    void Update()
    {
        ...
        Acore::Containers::EraseIf(_sockets, [this](std::shared_ptr<SocketType> const& sock)
        {
            if (!sock->Update())
            {
                if (sock->IsOpen())
...
private:
    ...
    Acore::Net::IoContextTcpSocket _acceptSocket;
    boost::asio::steady_timer _updateTimer;
};
}

#endif // ACORE_NET_NETWORK_THREAD_H
