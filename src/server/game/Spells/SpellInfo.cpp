...
SpellCastResult SpellInfo::CheckLocation(uint32 map_id, uint32 zone_id, uint32 area_id, Player* player /*= nullptr*/, bool strict /*= true*/) const
{
    ...
    if (HasAttribute(SPELL_ATTR4_ONLY_FLYING_AREAS) && (area_id || zone_id))
    {
        ...
        if (!areaEntry || !areaEntry->IsFlyable() || (strict && (areaEntry->Flags[0] & AREA_FLAG_NO_FLY_ZONE) != 0) || !player->canFlyInZone(map_id, zone_id, this))
        {
            return SPELL_FAILED_INCORRECT_AREA;
        }
    }
                ...
                if (Mechanic == MECHANIC_DISARM)
                {
                    ...
                    for (uint8 i = BASE_ATTACK; i < MAX_ATTACK; ++i)
                    {
                        ...
                        if (HasAura(disarmAuraType))
                        {
                            ...
                            else if (unitTarget->GetVirtualItemId(i))
                            {
                                valid = true;
                                break;
                            }
...
int32 SpellInfo::CalcPowerCost(Unit const* caster, SpellSchoolMask schoolMask, Spell* spell) const
{
    ...
    powerCost += caster->m_unitData->PowerCostModifier[school];
    ...
    powerCost = int32(powerCost * (1.0f + caster->m_unitData->PowerCostMultiplier[school]));
    if (powerCost < 0)
        powerCost = 0;
    return powerCost;
}
