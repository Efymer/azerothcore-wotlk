...
#ifndef ACORE_HTTP_SERVICE_H
#define ACORE_HTTP_SERVICE_H
...
class AC_COMMON_API DispatcherService
{
public:
...
class AC_COMMON_API SessionService
{
public:
...
private:
    ...
    std::unique_ptr<boost::asio::steady_timer> _inactiveSessionsKillTimer;

    std::string _logger;
};

template<typename Callable, typename SessionImpl>
concept HttpRequestHandler = Acore::invocable_r<Callable, RequestHandlerResult, std::shared_ptr<SessionImpl>, RequestContext&>;
