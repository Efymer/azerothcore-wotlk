...
bool Model::ConvertToVMAPModel(const char * outfilename)
{
    ...
    if (nIndexes > 0)
    {
        ...
        for (uint32 i = 0; i < nIndexes; ++i)
        {
            uint16 idx = static_cast<uint16>(indices[i]);
...
{
    std::string tempname = Acore::StringFormat("{}/{}", szWorkDirWmo, ModelInstName);
    ...
    auto extractSingleSet = [&](WMO::MODS const& doodadSetData)
    {
        for (uint16 doodadIndex : doodadData.References)
        {
            ...
            if (doodadData.Paths)
                ...
                ModelInstName = Acore::StringFormat("FILE{:08X}.xxx", doodadData.FileDataIds[doodad.NameIndex]);
            else
                ASSERT(false);
