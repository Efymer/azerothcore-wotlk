                    ...
                    switch (_loadInfo->Fields[fieldIndex].Type)
                    {
                        ...
                        case FT_STRING_NOT_LOCALIZED:
                            *reinterpret_cast<char const**>(&dataTable[offset]) = EmptyDb2String;
                            ...
                            break;
                        default:
                            ABORT("Unknown format character '{}' found in {} meta for field {}",
                    ...
                    switch (_loadInfo->Fields[fieldIndex].Type)
                    {
                        ...
                        case FT_SHORT:
                            *reinterpret_cast<uint16*>(&dataTable[offset]) = 0;
                            ...
                            break;
                        default:
                            ABORT("Unknown format character '{}' found in {} meta for parent field {}",
                                _loadInfo->Fields[fieldIndex].Type, _fileName, _loadInfo->Fields[fieldIndex].Name);
                            break;
                    }
                    ++fieldIndex;
...
char* DB2FileLoaderRegularImpl::AutoProduceStrings(char** indexTable, uint32 indexTableSize, uint32 locale)
{
    if (!(_header->Locale & (1 << locale)))
    {
        ...
        throw DB2FileLoadException(Acore::StringFormat("Attempted to load {} which has locales {} as {}. Check if you placed your localized db2 files in correct directory.",
                ...
                switch (_loadInfo->Meta->Fields[_loadInfo->Meta->ParentIndexField].Type)
                {
                    case FT_SHORT:
                    {
                        ...
                        else
                        {
                            ...
                            ASSERT(parentId <= std::numeric_limits<uint16>::max(), "ParentId value {} does not fit into uint16 field ({} in {})",
                    ...
                    case FT_BYTE:
                    {
                        ...
                        {
                            // in data block, must fit
                            ASSERT(parentId <= std::numeric_limits<uint8>::max(), "ParentId value {} does not fit into uint8 field ({} in {})",
                                parentId, _loadInfo->Fields[_loadInfo->GetFieldIndexByMetaIndex(_loadInfo->Meta->ParentIndexField)].Name, _fileName);
                    ...
                    case FT_INT:
                        *reinterpret_cast<uint32*>(&recordData[parentIdOffset]) = parentId;
                        break;
                    default:
                        ABORT("Unhandled parent id type '{}' found in {}", _loadInfo->Meta->Fields[_loadInfo->Meta->ParentIndexField].Type, _fileName);
                        break;
                }
            }
        }
...
uint32 DB2FileLoaderRegularImpl::RecordGetId(uint8 const* record, uint32 recordIndex) const
{
    if (_loadInfo->Meta->HasIndexFieldInData())
    {
        ...
        switch (_loadInfo->Meta->Fields[indexField].Type)
        {
            ...
            default:
                ABORT("Unhandled ID type {} in {}", uint32(_loadInfo->Meta->Fields[indexField].Type), _fileName);
        }
    }
...
T DB2FileLoaderRegularImpl::RecordGetVarInt(uint8 const* record, uint32 field, uint32 arrayIndex) const
{
    ...
    switch (compressionType)
    {
        ...
        default:
            ABORT("Unhandled compression type {} in {}", uint32(_columnMeta[field].CompressionType), _fileName);
            break;
    }
...
char* DB2FileLoaderSparseImpl::AutoProduceData(uint32& indexTableSize, char**& indexTable)
{
    if (_loadInfo->Meta->FieldCount != _header->FieldCount)
        throw DB2FileLoadException(Acore::StringFormat("Found unsupported parent index in sparse db2 {}", _fileName));
            ...
            for (std::size_t j = 0; j < _parentIndexes[i][0].Entries.size(); ++j)
            {
                ...
                switch (_loadInfo->Meta->Fields[_loadInfo->Meta->ParentIndexField].Type)
                {
                    case FT_SHORT:
                    {
                        ...
                        {
                            // in data block, must fit
                            ASSERT(parentId <= 0xFFFF, "ParentId value {} does not fit into uint16 field ({} in {})",
                                parentId, _loadInfo->Fields[_loadInfo->GetFieldIndexByMetaIndex(_loadInfo->Meta->ParentIndexField)].Name, _fileName);
                            *reinterpret_cast<uint16*>(&recordData[parentIdOffset]) = parentId;
                        }
                        break;
                    ...
                    case FT_BYTE:
                    {
                        ...
                        {
                            // in data block, must fit
                            ASSERT(parentId <= 0xFF, "ParentId value {} does not fit into uint8 field ({} in {})",
                                parentId, _loadInfo->Fields[_loadInfo->GetFieldIndexByMetaIndex(_loadInfo->Meta->ParentIndexField)].Name, _fileName);
                            *reinterpret_cast<uint8*>(&recordData[parentIdOffset]) = parentId;
                        }
                        break;
        ...
        for (uint32 arr = 0; arr < _loadInfo->Meta->Fields[field].ArraySize; ++arr)
        {
            ...
            switch (_loadInfo->Meta->Fields[field].Type)
            {
                ...
                case FT_STRING_NOT_LOCALIZED:
                    ...
                    break;
                default:
                    ABORT("Unknown format character '{}' found in {} meta", _loadInfo->Meta->Fields[field].Type, _fileName);
                    break;
            }
            ++combinedField;
        }
...
uint8 DB2Record::GetUInt8(char const* fieldName) const
{
    ...
    ASSERT(fieldIndex.first != -1, "Field with name {} does not exist!", fieldName);
...
void DB2FileLoader::LoadHeaders(DB2FileSource* source, DB2FileLoadInfo const* loadInfo)
{
    ...
    // The 3.4.3.54261 client ships WDC4 with no version/schema preamble: read the whole
    // header in one shot, matching the reference 3.4.3 core (xian55) byte-for-byte. (A prior
    // revision modelled a speculative WDC5 Version+Schema preamble in DB2Header, which shifted
    // every field by 132 bytes and corrupted the extractor's output — header-only db2 files.)
    ...
    if (_header.Signature != 0x34434457)                        //'WDC4'
        throw DB2FileLoadException(Acore::StringFormat("Incorrect file signature in {}, expected 'WDC4', got {}{}{}{}", source->GetFileName(),
    ...
    if (_header.ParentLookupCount > 1)
        throw DB2FileLoadException(Acore::StringFormat("Too many parent lookups in {}, only one is allowed, got {}",
    ...
    if (loadInfo && (_header.TotalFieldCount + (loadInfo->Meta->ParentIndexField >= int32(_header.TotalFieldCount) ? 1 : 0) != loadInfo->Meta->FieldCount))
        throw DB2FileLoadException(Acore::StringFormat("Incorrect number of fields in {}, expected {}, got {}",
    ...
    if (loadInfo && (_header.ParentLookupCount && loadInfo->Meta->ParentIndexField == -1))
        throw DB2FileLoadException(Acore::StringFormat("Unexpected parent lookup found in {}", source->GetFileName()));
    ...
    if (_header.SectionCount && !source->Read(sections.get(), sizeof(DB2SectionHeader) * _header.SectionCount))
        throw DB2FileLoadException(Acore::StringFormat("Unable to read section headers from {}", source->GetFileName()));
    ...
    if (!source->Read(fieldData.get(), sizeof(DB2FieldEntry) * _header.FieldCount))
        throw DB2FileLoadException(Acore::StringFormat("Unable to read field information from {}", source->GetFileName()));
    ...
    if (_header.ColumnMetaSize)
    {
        ...
        if (!source->Read(columnMeta.get(), _header.ColumnMetaSize))
            throw DB2FileLoadException(Acore::StringFormat("Unable to read field metadata from {}", source->GetFileName()));
        ...
        if (loadInfo && loadInfo->Meta->HasIndexFieldInData())
        {
            if (columnMeta[loadInfo->Meta->IndexField].CompressionType != DB2ColumnCompression::None
                ...
                throw DB2FileLoadException(Acore::StringFormat("Invalid compression type for index field in {}, expected one of None (0), Immediate (1), SignedImmediate (5), got {}",
        ...
        for (uint32 i = 0; i < _header.TotalFieldCount; ++i)
        {
            ...
            if (!source->Read(palletValues[i].get(), columnMeta[i].AdditionalDataSize))
                throw DB2FileLoadException(Acore::StringFormat("Unable to read field pallet values from {} for field {}", source->GetFileName(), i));
        }
        ...
        for (uint32 i = 0; i < _header.TotalFieldCount; ++i)
        {
            ...
            if (!source->Read(palletArrayValues[i].get(), columnMeta[i].AdditionalDataSize))
                throw DB2FileLoadException(Acore::StringFormat("Unable to read field pallet array values from {} for field {}", source->GetFileName(), i));
        }
        ...
        for (uint32 i = 0; i < _header.TotalFieldCount; ++i)
        {
            ...
            if (!source->Read(commonData[i].get(), columnMeta[i].AdditionalDataSize))
                throw DB2FileLoadException(Acore::StringFormat("Unable to read field common values from {} for field {}", source->GetFileName(), i));
...
void DB2FileLoader::Load(DB2FileSource* source, DB2FileLoadInfo const* loadInfo)
{
    ...
    for (uint32 i = 0; i < _header.SectionCount; ++i)
    {
        ...
        if (section.TactId)
        {
            ...
            if (!source->Read(&encryptedIdCount, sizeof(encryptedIdCount)))
                throw DB2FileLoadException(Acore::StringFormat("Unable to read number of encrypted records in {} for section {}", source->GetFileName(), i));
    ...
    if (loadInfo && !(_header.Flags & 0x1))
    {
        ...
        if (source->GetFileSize() != expectedFileSize)
            throw DB2FileLoadException(Acore::StringFormat("{} failed size consistency check, expected {}, got {}", source->GetFileName(), expectedFileSize, source->GetFileSize()));
    }
    ...
    for (uint32 i = 0; i < _header.SectionCount; ++i)
    {
        ...
        if (!source->SetPosition(section.FileOffset))
            throw DB2FileLoadException(Acore::StringFormat("Unable to change {} read position for section {}", source->GetFileName(), i));

        if (!_impl->LoadTableData(source, i))
            throw DB2FileLoadException(Acore::StringFormat("Unable to read section table data from {} for section {}", source->GetFileName(), i));

        if (!_impl->LoadCatalogData(source, i))
            throw DB2FileLoadException(Acore::StringFormat("Unable to read section catalog data from {} for section {}", source->GetFileName(), i));
        ...
        if (loadInfo)
        {
            if (loadInfo->Meta->HasIndexFieldInData())
            {
                if (section.IdTableSize != 0)
                    throw DB2FileLoadException(Acore::StringFormat("Unexpected id table found in {} for section {}", source->GetFileName(), i));
            }
            else if (section.IdTableSize != 4 * section.RecordCount)
                throw DB2FileLoadException(Acore::StringFormat("Unexpected id table size in {} for section {}, expected {}, got {}",
        ...
        if (section.IdTableSize)
        {
            ...
            if (!source->Read(&idTable[idTableSize], section.IdTableSize))
                throw DB2FileLoadException(Acore::StringFormat("Unable to read non-inline record ids from {} for section {}", source->GetFileName(), i));
        ...
        if (!(_header.Flags & 0x1) && section.CopyTableCount)
        {
            ...
            if (!source->Read(&copyTable[copyTableSize], section.CopyTableCount * sizeof(DB2RecordCopy)))
                throw DB2FileLoadException(Acore::StringFormat("Unable to read record copies from {} for section {}", source->GetFileName(), i));
        }
        ...
        if (_header.ParentLookupCount)
        {
            ...
            for (uint32 j = 0; j < _header.ParentLookupCount; ++j)
            {
                ...
                if (!source->Read(&indexInfo, sizeof(DB2IndexDataInfo)))
                    throw DB2FileLoadException(Acore::StringFormat("Unable to read parent lookup info from {} for section {}", source->GetFileName(), i));
                ...
                if (!source->Read(parentIndexesForSection[j].Entries.data(), sizeof(DB2IndexEntry) * indexInfo.NumEntries))
                    throw DB2FileLoadException(Acore::StringFormat("Unable to read parent lookup content from {} for section {}", source->GetFileName(), i));
            }
        }
    }
    ...
    if (loadInfo)
    {
        ...
        if (!loadInfo->Meta->HasIndexFieldInData())
        {
            if (loadInfo->Fields[0].IsSigned)
                signValidationResult.emplace_back(Acore::StringFormat("ID must be unsigned in {}", source->GetFileName()));
            ++fieldIndex;
        }
        for (uint32 f = 0; f < loadInfo->Meta->FieldCount; ++f)
        {
            ...
            {
                signValidationResult.emplace_back(Acore::StringFormat("Field {} in {} must be {}", loadInfo->Fields[fieldIndex].Name,
        ...
        if (!signValidationResult.empty())
            throw DB2FileLoadException(Acore::StringFormat("{}", fmt::join(signValidationResult, "\n")));
    }
}
