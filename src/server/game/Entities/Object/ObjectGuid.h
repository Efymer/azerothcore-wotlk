...
// [3.4.3.54261] 14-type client object layout. Values MUST match the client: UF holder block-bits and the
// SMSG_UPDATE_OBJECT changed-object-type mask are indexed by these TYPEID values. The Azerite/AreaTrigger/
// SceneObject/Conversation ids have no AzerothCore entity but are reserved so the wire layout stays in parity.
enum TypeID
{
    TYPEID_OBJECT                 = 0,
    TYPEID_ITEM                   = 1,
...
// TYPEMASK_x == (1 << TYPEID_x). Server-internal bitfield used by Object::isType(); not sent to the client.
enum TypeMask
{
