        ...
        for (uint8 arr = 0; arr < _loadInfo->Meta->Fields[i].ArraySize; ++arr)
        {
            switch (_loadInfo->Meta->Fields[i].Type)
            {
                ...
                case FT_LONG:
                    ...
                    break;
                case FT_STRING:
                    buffer << reinterpret_cast<LocalizedString const*>(entry)->Str[locale];
...
void DB2StorageBase::LoadStringsFrom(std::string const& path, LocaleConstant locale)
{
    ...
    if (!_indexTable)
        throw DB2FileLoadException(Acore::StringFormat("{} was not loaded properly, cannot load strings", path));
