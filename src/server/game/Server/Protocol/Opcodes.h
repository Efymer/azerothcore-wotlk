...
public:
    ...
    bool IsValid(OpcodeClient index) const
    {
        ...
        return opcodeArrayIndex >= 0 && opcodeArrayIndex < std::ptrdiff_t(NUM_CMSG_OPCODES);
    }

    bool IsValid(OpcodeServer index) const
    {
        std::ptrdiff_t opcodeArrayIndex = GetOpcodeArrayIndex(index);
        return opcodeArrayIndex >= 0 && opcodeArrayIndex < std::ptrdiff_t(NUM_SMSG_OPCODES);
    }

    ClientOpcodeHandler const* operator[](OpcodeClient index) const
    {
        return _internalTableClient[GetOpcodeArrayIndex(index)];
    }

    ServerOpcodeHandler const* operator[](OpcodeServer index) const
    {
        return _internalTableServer[GetOpcodeArrayIndex(index)];
    }
...
private:
    ...
    ClientOpcodeHandler* _internalTableClient[NUM_CMSG_OPCODES];
    ServerOpcodeHandler* _internalTableServer[NUM_SMSG_OPCODES];
