...
// Field types used by the DB2/DBC metadata when interpreting records.
// Values are kept compatible with TrinityCore's DB2 store definitions so that
// ported per-table metadata interprets the same format characters.
enum DBCFormer
{
    FT_NA       = 'x',                                       // not used or unknown, 4 byte size
    FT_NA_BYTE  = 'X',                                       // not used or unknown, byte
    FT_STRING   = 's',                                       // localized char*
    FT_STRING_NOT_LOCALIZED = 'S',                           // char* not loaded to locale set
    FT_FLOAT    = 'f',                                       // float
    FT_INT      = 'i',                                       // uint32
    FT_BYTE     = 'b',                                       // uint8
    FT_SHORT    = 'h',                                       // uint16
...
{
    bool HasString(LocaleConstant locale = DEFAULT_LOCALE) const
    {
        return Str[locale][0] != '\0';
    }
...
struct AC_COMMON_API DB2FileLoadInfo
{
...
struct AC_COMMON_API DB2FileSource
{
    DB2FileSource();
...
class AC_COMMON_API DB2FileLoadException : public std::exception
{
public:
...
class AC_COMMON_API DB2FileLoader
{
public:
    DB2FileLoader();
