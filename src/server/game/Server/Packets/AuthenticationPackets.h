...
namespace WorldPackets
{
    namespace Auth
    {
        // TODO(3.4.3 brick-C): these mirror the game-data structures TrinityCore pulls from
        // ObjectMgr / CharacterTemplateDataStore. AzerothCore has no equivalent yet, so they are
        // defined locally and left empty by default. The wire format is faithful; only the data
        // sources are stubbed (see brick D / login-sequence).
        ...
        public:
            struct AuthSuccessInfo
            {
                ...
                std::vector<RaceClassAvailability> const* AvailableClasses = &EmptyAvailableClasses; ///< the minimum AccountExpansion required to select race/class combinations
            ...
            // TODO(3.4.3 brick-C): populate AvailableClasses/Templates/VirtualRealms from server data (brick D / login-sequence)
            static std::vector<RaceClassAvailability> const EmptyAvailableClasses;
        };
        ...
        class AC_GAME_API ConnectTo final : public ServerPacket
        {
        public:
        ...
        class AC_GAME_API EnterEncryptedMode final : public ServerPacket
        {
        public:
            static bool InitializeEncryption();
            static void ShutdownEncryption();
